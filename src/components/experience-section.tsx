      ...
      <div>
        <ol className="group/list">
          {RESUME_DATA.work.map((work, workIndex) => (
            <li key={workIndex} className="mb-12">
              {work.roles.map((role, roleIndex) => (
                <div
                  key={roleIndex}
                  className="group relative grid pb-1 transition-all sm:grid-cols-8 sm:gap-8 md:gap-4 lg:hover:!opacity-100 lg:group-hover/list:opacity-50 mb-8 last:mb-0"
                >
                  ...
                  <header
                    ...
                    aria-label={`${role.start} to ${role.end ?? "Present"}`}
                  >
                  ...
                  <div className="z-10 sm:col-span-6">
                    <h3 className="font-medium leading-snug text-foreground">
                      <div>
                        <a
                          ...
                          aria-label={`${role.title} at ${work.company}`}
                        >
                    ...
                    {roleIndex === 0 && work.badges.length > 0 && (
                      <ul className="mt-2 flex flex-wrap" aria-label="Technologies used">
                        {work.badges.map((badge) => (
                          <li key={badge} className="mr-1.5 mt-2">
